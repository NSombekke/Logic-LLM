...
Most current DMs implement a U-Net architecture, an autoencoder-like network used to predict the noise added at a particular stage of the diffusion process. Asyrp discovers semantic meaning in the bottleneck of the U-Net. Augmentation of this bottleneck, $\mathbf{\Delta h_{t}}$, is predicted by an implicit neural function with two inputs - the bottleneck feature maps, $\mathbf{h_{t}}$, and the time step $t$. This process of editing the bottleneck results in finding what the authors call h-space, a semantic latent space displaying the properties of homogeneity, linearity, robustness, and consistency across time steps.
